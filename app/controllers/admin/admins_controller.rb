class Admin::AdminsController < Admin::BaseController
  ...
  def questions
    @questions = Question.includes(:user).published
  end

  def answers
    @answers = Answer.includes(:user, :question, :rich_text_content).published
  end

  def comments
    @comments = Comment.includes(:user, :commentable).published
  end
